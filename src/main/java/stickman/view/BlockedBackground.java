...
public class BlockedBackground implements BackgroundDrawer {
    ...
    @Override
    public void update(double xViewportOffset,double yPosToChange) {
        // do nothing since this is a static bg
        double floorHeight = model.getCurrentLevel().getFloorHeight();
        this.sky.setHeight(floorHeight+yPosToChange);
        this.floor.setY(floorHeight+yPosToChange);
        this.floor.setHeight(this.pane.getHeight()-floorHeight-yPosToChange);
    }
}
